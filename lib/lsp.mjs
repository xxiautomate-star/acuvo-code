...
/**
 * Where is this language's server, if anywhere?
 *
 ...
 */
export function discoverLanguageServer(root, language, { env = process.env } = {}) {
  ...
  if (spec.binary) {
    ...
    if (found?.ok) {
      return { ok: true, language, label: spec.label, file: found.file, argv: [...spec.args], via: found.file };
    }
    if (found?.shim) {
      return {
        ok: false,
