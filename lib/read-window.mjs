...
/**
 * ⚠️ STRICTER THAN `read_file`, DELIBERATELY — FLAG THIS WHEN WIRING IT.
 *
 * `executor.readFile` will happily hand a model `.env.local`. This one will not.
 * The divergence is intentional and it is not a claim that `read_file` is fine:
 * a windowed reader is the tool a model reaches for when it is EXPLORING, which
 * is precisely when it stumbles into a credential file it had no reason to open.
 * Narrowing the new door is free; narrowing the old one changes behaviour other
 * things depend on, so that is a separate, deliberate decision — not something
 * to slip in under a different patch.
 *
...
function credentialRefusal(base) {
  if (!CREDENTIAL_BASENAME.some((rx) => rx.test(base))) return null;
