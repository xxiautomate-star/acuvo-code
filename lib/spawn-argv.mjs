...
export async function runProgram({ root, program, args, timeoutMs, spawnImpl } = {}) {
  ...
  let planned;
  if (program === 'node') planned = buildNodeArgv(cwd, list.args);
  else if (program === 'npx') planned = buildNpxArgv(cwd, list.args);
  else planned = buildCheckedArgv(cwd, 'tsc', list.args, 'tsc argument');
...
/** `npm` never actually gets spawned — see runPackageScript for why. */
async function runNpm({ root, args, timeoutMs, spawnImpl }) {
  const sub = args[0];
  ...
  return [
    {
      ...
      function: {
        ...
        description: [
          ...
          'untouched. npm: only test, run <script>, and run <script> -- <args…>; install/ci/exec/publish',
          'are refused. npx: only vitest and tsc, already installed. A glob (* or ?) in a node test',
