  ...
  function canContinue() {
    ...
    if (remainingUsd <= USD_EPSILON) {
      return {
        ...
        message: `budget spent: ${formatUsd(spentUsd)} of ${formatUsd(limitUsd)} after ${rounds.length} round${rounds.length === 1 ? '' : 's'}.`,
    ...
    if (spentUsd + projection.usd > effectiveLimit + USD_EPSILON) {
      ...
      return {
        ...
        message: tooSmall
          ...
          : `stopping on budget after ${rounds.length} round${rounds.length === 1 ? '' : 's'}: ${formatUsd(spentUsd)} of ${formatUsd(limitUsd)} spent and the next round is projected at ~${formatUsd(projection.usd)}, which would cross the line.`,
        remainingUsd,
        ...base,
      };
    }
