...
import { executeToolCall, toolSchemasFor, SINGLE_SHOT_TOOL_NAMES, toolNamesForRounds, TOOL_NAMES } from './tools.mjs';
...
import {
  freshWarmth, rememberWarm, forgetWarm, routeFor, describeRouting, loadWarmth, saveWarmth,
...
import { wrapUntrusted } from './untrusted-block.mjs';
 ...
 */
import { loadPlan, formatBanner, planFileFor, planTaskRelation, foreignPlanNotice } from './plan-ledger.mjs';
/**
 ...
 */
import { skillsPromptBlock, formatSkillForModel } from './skills.mjs';
...
import { shortlistTools, shouldWiden } from './tool-shortlist.mjs';
 ...
 * @returns {string}
 */
export function assembleSystemMessage({ base, memoryBlock, skillsBlock, learnedBlock } = {}) {
  /**
...
export function toolResultText(record) {
  ...
  const notes = failures.map((f) => {
    const what = f.kind === 'error'
      ...
      : `exited ${f.exitCode}`;
    return `[PostToolUse hook "${f.hook}" ${what}]${f.output ? `\n${f.output}` : ''}`;
  });
  return `${body}\n${notes.join('\n')}`;
}
...
function describeToolResult(record) {
  ...
  switch (name) {
     ...
     */
    case 'see_page':
      return result.verdict ? String(result.verdict) : stringifyForModel(result, 2_000);
...
export function renderEvent(event) {
  switch (event.type) {
    ...
    case 'tool-start': {
      ...
      const subject = a?.command ?? a?.entry ?? a?.query ?? a?.url ?? a?.path ?? a?.pattern ?? a?.prompt ?? null;
      return [paint.dim(`  … ${event.name}${subject ? ` ${String(subject).slice(0, 60)}` : ''}`)];
    }
    case 'tool':
...
export async function runSession({
   ...
   */
  /**
   * ── ⚠️⚠️ OFF BY DEFAULT, AND THE REASON IS A GUARD THAT CAUGHT ME ─────────
   *
   * Wiring this on by default turned `tool-prefix-order.test.mjs` red: two
   * sessions differing only in `--no-run` shared 14,088 bytes of prefix against
   * an absolute floor of 15,000. The guard is protecting the exact cache
   * measured today, and it is right to.
   *
   * ⭐ THE TRADE IS REAL IN BOTH DIRECTIONS, which is why it is not mine to
   * make silently. Shortlisting cuts the per-round payload by ~56% (46,609 →
   * 20,430 chars on a focused brief), and PROPORTIONALLY it shares MORE of its
   * head — 69% of the shortlisted block versus 32% of the full one. But the
   * floor is absolute bytes, and a smaller block cannot reach an absolute floor
   * set for a larger one.
   *
   * ⚠️ Rewriting that guard so my own change passes is precisely the move this
   * repo keeps paying for. So the capability ships measured and OFF;
   * `ACUVO_TOOL_SHORTLIST=1` turns it on, and turning it on is a decision about
   * whether ~6,500 tokens a round is worth a shorter shared head.
   */
  const shortlistEnabled = String(process.env.ACUVO_TOOL_SHORTLIST ?? '').trim() === '1';
  ...
  if (!continuing && offered.includes('read_skill')) {
    try {
      skillsBlock = skillsPromptBlock(discoverAllSkills(executor.root));
    } catch {
      skillsBlock = null;
    }
  }
  ...
  const systemText = assembleSystemMessage({
    base: systemPrompt({ maxRounds, allowRun, offeredNames: offered, untilDone }),
    memoryBlock,
    skillsBlock,
    learnedBlock,
  });
  ...
  for (let round = 1; round <= maxRounds; round += 1) {
     ...
     */
    const banner = planBannerFor(executor, { roundIndex: round, maxRounds, task });
    if (banner) {
     ...
     * does not exist.
     */
    const wire = appendOnlyWireBytes({ tools, messages });
     ...
     */
    if (reply?.ok && reply?.provider) {
      const landing = describeRouting({ expected: route.strict ? route.order[0] : null, served: reply.provider });
      if (landing.warm === false) forgetWarm(warmth, config.model);
      rememberWarm(warmth, config.model, reply.provider);
    } else if (!reply?.ok) {
      forgetWarm(warmth, config.model);
    }
    /**
    ...
    for (const call of calls) {
      ...
      const record = ns
        ...
        : await executeToolCall(call, executor, {
            ...
            shell,
            /**
             * ⭐ `task` RIDES WITH THE ROUND NUMBERS, and it is what lets a plan
             * record WHICH TASK IT BELONGS TO. `tools.mjs` already spreads this
             * object straight into the plan tools' options, so the runner's task
             * reaches `plan_start` with no new plumbing — and a plan that knows
             * its own task cannot be injected into an unrelated later run.
             *
             * ⚠️ It is HASHED before it touches disk (`taskKey`), never stored:
             * the user's prompt is not the model's words, so a secret in it must
             * not become a file, and must not be able to refuse the call either.
             */
            ...
            ask: askUser,
            /**
             * ⭐ WRITE REVIEW — built ONCE per turn, because the whole point is
             * the memory it carries between rounds: which files this run
             * created (revising your own draft is not a change to review) and
             * whether "all remaining" was already answered.
             */
            approveWrite: writeApprover.approve,
            approveBatch: writeApprover.approveMany,
            /**
             * ⚠️ THE BUDGET ITSELF, not a number. `delegate` needs to ask what
             * is LEFT at the moment it runs and to charge the helper's spend
             * back when it returns — a value captured here would be stale by
             * the round the model actually delegates.
             */
            budget,
          });
  ...
  for (const command of staleCommands) {
     ...
     */
    const staleCall = { id: 'final_check', function: { name: 'run_command', arguments: JSON.stringify({ command }) } };
  ...
  const usage = aggregateUsage(rounds, prefixReadings);
  /**
...
function aggregateUsage(rounds, prefixReadings = []) {
  let cost = 0;
  let tokens = 0;
  let seen = false;
  for (const r of rounds) {
  ...
  const prefix = scored.length > 0
    ...
    : null;
  return seen ? { cost, total_tokens: tokens, cache: cache ? { ...cache, prefix } : (prefix ? { prefix } : null) } : null;
}
...
function cacheClause(cache) {
  ...
  const firstClause = first && typeof first.hitRate === 'number' && first.promptTokens > 0
    ...
    : '';
  return ` · cache ${pct}% (${cache.cachedTokens} of ${cache.promptTokens} prompt tokens${firstClause}${partial})`;
}
...
function providerPinWarning(outcome) {
  ...
  if (p.pinMissed === 0 && fellBack === 0) return [];
...
export function formatSummary(outcome) {
  ...
  if (applied.length === 0) {
    ...
    lines.push(...formatChanges(applied.flatMap(describeChanges)));
  }
