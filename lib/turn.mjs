...
import { clampOutput, formatRunForModel, DEFAULT_COMMAND_TIMEOUT_MS, ALLOWED_BINARIES } from './command.mjs';
/**
 ...
 */
import { loadPlan, formatBanner } from './plan-ledger.mjs';
/**
 ...
 */
import { compactMessages } from './compact.mjs';
 ...
 */
export function systemPrompt({ maxRounds = 1, allowRun = true, offeredNames = [] } = {}) {
  if (maxRounds > 1) return loopSystemPrompt(maxRounds, allowRun, offeredNames);
  return [
 ...
 */
function loopSystemPrompt(maxRounds, allowRun, offeredNames = []) {
  return [
    'You are Acuvo Code, a coding agent running in a terminal on the user\'s own machine.',
    '',
...
/**
 * The ceiling the conversation is held under, in ESTIMATED tokens (chars/4 —
 * counting properly would mean an npm dependency, and zero-deps is this
 * package's headline property). 24,000 leaves ample room under every provider
 * in the chain while being low enough that a genuinely long session hits it.
 */
...
export function toolResultText(record) {
  ...
  switch (name) {
    ...
    /* c8 ignore next 2 */
    default:
      return clampOutput(JSON.stringify(result), 2_000).text;
  }
}
 ...
 * @typedef {{ round: number, note?: string | null, executed: ToolRecord[], usage: any, finishReason?: string | null, error?: string }} RoundRecord
 ...
 * @typedef {{ ok: true, stage: 'done', model: string, note: string | null, finishReason: string | null, usage: { cost?: number, total_tokens?: number } | null, executed: ToolRecord[], rounds: RoundRecord[], roundsUsed: number, maxRounds: number, allowRun: boolean, stoppedBecause: string, verification: Verification, acceptance: Acceptance | null, promisedButMissing: string[] }} SessionDone
...
export async function runSession({
  ...
  const context = continuing
    ? { ok: true, text: '' }
    : gatherWorkspaceContext(executor);
  if (!context.ok) {
   ...
   */
  const offered = toolNames ?? toolNamesForRounds(maxRounds, { allowRun, root: executor.root });
  /**
   ...
   */
  const mcpAllowed = maxRounds > 1 && allowRun && !executor.dryRun;
  if (mcpAllowed) {
  ...
  const tools = [...toolSchemasFor(offered), ...mcpSchemas];
  /**
  ...
  const preamble = [memoryBlock, skillsBlock].filter(Boolean).join('\n\n');
  const systemText = preamble
    ? `${preamble}

${systemPrompt({ maxRounds, allowRun, offeredNames: offered })}`
    : systemPrompt({ maxRounds, allowRun, offeredNames: offered });
  ...
  for (let round = 1; round <= maxRounds; round += 1) {
     ...
     */
    const live = createLivePrinter({ write: (t) => onEvent({ type: 'stream', text: t }) });
     ...
     */
    const fit = compactMessages(messages, { budgetTokens: CONTEXT_BUDGET_TOKENS, keepLastRounds: 2 });
    if (fit.dropped > 0) {
      messages.length = 0;
    ...
    onEvent({ type: 'note', round, text: reply.content, streamed: live.linesPrinted() > 0 });
  ...
  const writtenPaths = executed.filter((e) => e.mutated).map((e) => e.mutatedPath ?? e.result.path);
...
function aggregateUsage(rounds) {
  ...
  return seen ? { cost, total_tokens: tokens } : null;
}
...
export function formatSummary(outcome) {
  ...
  if (typeof cost === 'number') {
    ...
    lines.push('');
    lines.push(`${outcome.model}${roundsPart} · ${tokens ?? '?'} tokens · $${cost.toFixed(6)}`);
  }
 ...
 */
export function sessionFailed(outcome) {
