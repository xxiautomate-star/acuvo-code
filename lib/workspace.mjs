...
export function resolveInWorkspace(root, raw, intent = 'read') {
  ...
  if (intent === 'write' && WRITE_FORBIDDEN_ROOTS.has(segments[0])) {
    return { ok: false, reason: `writing into ${segments[0]}/ is refused — it executes code on the owner's next command` };
  }
 ...
 */
export function createLocalExecutor(root, { dryRun = false } = {}) {
