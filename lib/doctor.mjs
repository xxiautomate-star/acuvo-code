...
import { mediaConfig } from './media.mjs';
import { imageConfig, generateEndpoint, IMAGE_URL_ENV, IMAGE_TOKEN_ENV } from './imagegen.mjs';
...
import { gitStatus as realGitStatus } from './git.mjs';
...
const RUN_GATED = new Set(['run_command', 'run_program', 'evaluate', 'git_commit', 'declare_acceptance', 'check_acceptance']);
...
export function toolOffer({ root = process.cwd(), env = process.env, allowRun = true, maxRounds = 8 } = {}) {
  ...
  for (const name of TOOL_NAMES) {
    ...
    withheld.push({ name, ...whyWithheld(name, { env, allowRun, maxRounds, languages, failed }) });
  }
  return { offered, withheld, total: TOOL_NAMES.length };
}

function whyWithheld(name, { env, allowRun, maxRounds, languages, failed }) {
  if (failed) {
  ...
  if (maxRounds <= 1 && !MEDIA_ENV[name] && name !== 'generate_image') {
    return {
      ...
      fix: 'raise the round budget (--rounds N) so results have somewhere to go',
    };
  }
  ...
  if (name === 'read_skill') {
    return {
      why: `no ${ACUVO_DIR}/skills directory with at least one readable skill in it`,
      fix: `create ${ACUVO_DIR}/skills/<name>.md with name/description frontmatter`,
    };
  }
...
export function mcpChecks(cfg, env = {}, { resolve = resolveMcpCommand } = {}) {
  ...
  if (!cfg.file || cfg.servers.length === 0) {
    return [{
      ...
      verified: true,
      detail: cfg.file
        ? `${cfg.file} declares no servers, so the model sees only the ${TOOL_NAMES.length} tools built into this CLI`
        : `no ${names} in this workspace, so the model sees only the ${TOOL_NAMES.length} tools built into this CLI`,
...
export function cacheChecks({ model, maxRounds = 8 } = {}) {
  ...
  if (support.kind === 'automatic') {
    checks.push({
       ...
       * about the MODEL, read from a table — nothing here measured your run.
       * The hit rate itself is not instrumented yet, and a green tick claiming
       * otherwise is exactly the false all-clear this file exists to prevent.
       */
      verified: false,
      detail: `${model} caches the prompt prefix automatically — a continuing session re-sends an identical prefix and is billed a fraction for it (measured on DeepSeek: 97.2% cached, 3-4x cheaper). This is the model's documented behaviour, not a measurement: the real hit rate is not instrumented yet.`,
      fix: null,
    });
  ...
  if (maxRounds <= 1) {
    checks.push({
      ...
      fix: 'raise the round budget (--rounds N) — caching only pays from the second round onward',
    });
  }
  return checks;
}
...
export async function runDoctor({
  ...
  const probed = typeof fetchImpl === 'function';
  ...
  const renderVar = env.RENDER_AUDIT_URL?.trim() ? 'RENDER_AUDIT_URL'
    : env.MODAL_RENDER_AUDIT_URL?.trim() ? 'MODAL_RENDER_AUDIT_URL'
      : 'RENDER_AUDIT_URL';
  const mediaTargets = [
    { id: 'see_page', label: 'see_page', envVar: renderVar, url: media.render },
    { id: 'speak', label: 'speak', envVar: 'MODAL_TTS_URL', url: media.speak },
    { id: 'transcribe', label: 'transcribe', envVar: 'MODAL_TRANSCRIBE_URL', url: media.transcribe },
    { id: 'make_document', label: 'make_document', envVar: 'MODAL_PRESS_URL', url: media.document },
  ];
  ...
  for (const t of mediaTargets) {
    if (!t.url) {
      mediaChecks.push({
        ...
        state: 'dark',
        verified: true,
        detail: `${t.envVar} is unset, so ${t.label} is never offered to the model`,
        fix: `set ${t.envVar} to your endpoint URL (and MODAL_VIDEO_SECRET to the value it expects)`,
      });
      continue;
    }
  ...
  mediaChecks.push(assessImage(image, results.image, offline, probed));
  ...
  sections.push({ id: 'mcp', title: 'MCP servers', checks: mcpChecks(mcp, env) });
...
function assessImage(image, probe, offline, probed) {
  ...
  const host = hostOf(image.base);
  const where = host ? ` (${host}${image.usingDefault ? ', the built-in default' : ''})` : '';
