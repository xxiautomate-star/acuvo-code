...
import { gitStatus, gitDiff, gitLog, gitCommit, gitToolSchemas } from './git.mjs';
import { speak, transcribe, makeDocument, mediaToolSchemas, mediaToolNames } from './media.mjs';
/**
...
import { planStart, planStep, planStatus, planToolSchemas } from './plan-ledger.mjs';
...
TOOL_SCHEMAS.push(...searchToolSchemas());
 ...
 */
export function toolNamesForRounds(maxRounds, { allowRun = true, env = process.env, root = process.cwd() } = {}) {
  /**
   ...
   */
  if (lspAvailable(root, env)) names.push(...LSP_TOOL_NAMES);

  return withImage(names);
}

/** The subset of the registry to put in a request payload. */
export function toolSchemasFor(names) {
  const wanted = new Set(names);
  return TOOL_SCHEMAS.filter((t) => wanted.has(t.function.name));
}
 ...
 */
export async function executeToolCall(call, executor, { commandTimeoutMs, round = null, allowRun = true } = {}) {
  ...
  switch (name) {
    ...
    case 'transcribe': {
      const result = await transcribe(executor.root, args.path);
    ...
    case 'run_command': {
      ...
      const result = typeof executor.runCommand === 'function'
        ...
        : await executeRunCommand({ command: args.command, executor, timeoutMs: commandTimeoutMs });
      /**
     ...
     */
    case 'plan_start':
      return { id, name, args, result: planStart(executor.root, args, round ?? {}), mutated: false };
    case 'plan_step':
      return { id, name, args, result: planStep(executor.root, args, round ?? {}), mutated: false };
    case 'plan_status':
      ...
      return { id, name, args, result: planStatus(executor.root, round ?? {}), mutated: false };
